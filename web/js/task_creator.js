let collapseCardStartId1HTML =
  "<!-- Collapsable Card Example -->" +
  '<div class="card shadow mb-4">' +
  '<a href="#';
// -> Insert ID
let collapseCardEndId1StartId2HTML =
  '" class="d-block card-header py-3" data-toggle="collapse" ' +
  'role="button" aria-expanded="true" aria-controls="';
// -> Insert ID again
let collapseCardEndId2StartHeaderHTML =
  '">' + '<h6 class="m-0 font-weight-bold text-primary">';
// -> Insert Header
let collapseCardEndHeaderStartId3HTML =
  "</h6>" +
  "</a>" +
...
function noResultsVisualization() {
  const text =
    "Oops... We did not find any suitable results. Try something else.";
  const html =
    '<img class="mx-auto d-block" style="width: 25rem; margin-top: 1rem; margin-bottom: 1rem;" ' +
      ...
      .then((response) => {
        ...
        response.json().then((data) => {
          ...
          if (result[0].length !== 0) {
            state.text = text;
            state.task = task;
            state.specifiedTask = specifiedTask;
...
function createTaskByResult(task, result) {
  ...
  if (task === "PASSIVE_VOICE") {
    ...
    outputExercise(
      passivePhrases,
      passivePhrasesLexemes,
      passivePhrasesIndices,
      passivePhrasesSents
    );
  }
  resizeInputs();
...
function updateTask(server, text, task, specifiedTask) {
  return new Promise((resolve, reject) => {
    ...
    newTaskRequest(server, text, task, specifiedTask)
      .then(function () {
        $("#put_text").empty();
...
function collapseCardWrapper(taskID, task) {
  return (
    collapseCardStartId1HTML +
    "collapseCard-" +
    taskID +
    collapseCardEndId1StartId2HTML +
    "collapseCard-" +
    taskID +
    collapseCardEndId2StartHeaderHTML +
    "Task " +
    taskID +
    collapseCardEndHeaderStartId3HTML +
    "collapseCard-" +
    taskID +
    collapseCardEndId3StartTaskHTML +
    task +
    collapseCardEndTaskHTML
  );
}
  ...
  if (posWords !== null && posWords.length > 0) {
    ...
    for (let word of setOfWords) {
      ...
      node = document.createTextNode(
        count.toString() + ") " + word.toUpperCase()
      );
      para.appendChild(node);
      element = document.getElementById("put_text");
      if (element !== null) element.appendChild(para);
      count += 1;
    }
  }
}
...
function outputExercise(
  ...
  if (phrases !== null && phrases.length > 0) {
    ...
    for (let i = 0; i < phrases.length; i++) {
      ...
      if (is_different) {
        cur_sent = phrases_sents[i];
        processed_sentence = '';
        // We need a button id related to all the correct phrases ids.
        // Example: if we have 2 phrases in the same sentence with their ids = 9, 10,
        // we create Button Id = 'task-9-10' and so on.
        checkButton =
          '<div class="task-btn"><button type="button" class="btn btn-outline-primary btn-check-task", id="task-';
        left_index = 0;
      }
      ...
      for (let j = 0; j < phrases[i].length; j++) {
        let right_index = phrases_indices[i][j][0];
      ...
      if (
        ...
        processed_sentence =
          '<div class="task-p"><p>' +
          processed_sentence +
          cur_sent.substring(left_index, cur_sent.length) +
          "</p></div>" +
          checkButton +
          "<hr>";
    ...
    let b =
      '<button type="button" class="btn btn-outline-primary btn-lg btn-check-all", id="task-' +
      i.toString() +
